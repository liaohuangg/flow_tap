...
checkpoint="${CHECKPOINT:-$repo_root/LayoutGenModel/logs/model/placement_first_300k/placement-300k-ablation-no-thermal-no-wirelength/seed_61/latest.ckpt}"
chiplet_area_ratio="${CHIPLET_AREA_RATIO:-0.50}"
method="${METHOD:-cases-hubump-util50-ablation-no-thermal-no-wirelength}"
...
echo "Ablation checkpoint: $checkpoint"
echo "Thermal guidance: OFF"
echo "Neural wirelength and HPWL guidance: OFF"
