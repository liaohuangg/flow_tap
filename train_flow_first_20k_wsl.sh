...
# Flow-matching training on the 20k post-processed placement set.
#
# Dataset: Dataset/dataset/placement_dataset/placement_dataset_opt (20,000 systems),
# split by Dataset/splits/placement_opt_20k into 16000 train / 2000 val / 2000 test.
# Those layouts were re-derived so the graph topology, wireCount and per-chiplet
# power follow the layout instead of being independent random values, which is
# what makes the condition -> layout mapping learnable at all.
#
...
#   * task is overridden to placement_opt_20k, so the dataset, the log directory
...
#   * Checkpoints land in LayoutGenModel/checkpoints/model_opt, not logs/.  The
#     trainer always inserts model/<task>/<method>/seed_<seed> under log_root, so
#     the run is symlinked into the flat model_opt/placement_opt_20k/seed_61
...
monitor_every=100
print_every=500
method="placement-20k-thermal-wirelength-legality"
...
# placement_opt_20k/seed_61.  Needed when the point of the run is to compare
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --no-aux-t-reweight) aux_t_reweight=false ;;
    --aux-t-reweight) aux_t_reweight=true ;;
    --fresh) fresh=true ;;
...
required_paths=(
  ...
  "$repo_root/Dataset/splits/placement_opt_20k/manifest.yaml"
  "$repo_root/Dataset/dataset/placement_dataset/placement_dataset_opt/chiplet_dataset_1.json"
)
for required_path in "${required_paths[@]}"; do
  [[ -e "$required_path" ]] || { echo "Required training input is missing: $required_path" >&2; exit 1; }
done

log_root="$layout_root/checkpoints/model_opt"
run_dir="$log_root/model/placement_opt_20k/$method/seed_61"
if [[ "$fresh" == true ]]; then
  # Give a fresh run its own directory.  Without this the resume block below
  # would find the *previous* run sitting at placement_opt_20k/seed_61, move it
  ...
  final_dir="$log_root/placement_opt_20k/$method/seed_61"
else
  final_dir="$log_root/placement_opt_20k/seed_61"
fi
...
echo "Dataset: Dataset/dataset/placement_dataset/placement_dataset_opt (16000 train / 2000 val)"
train_layouts=16000
