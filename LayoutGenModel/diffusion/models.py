...
def _progress_from_t(t):
    # Samplers run from t ~= 1 to t ~= 0, so progress is elapsed sampling fraction.
    return min(1.0, max(0.0, 1.0 - _as_float(t)))
...
class CondDiffusionModel(nn.Module):
    backbones = {
        ...
        "geometry_att_gnn": networks.GeometryAttGNN,
    ...
    def __init__(
        ...
        elif backbone in ["res_gnn_block", "res_gnn", "graph_unet", "att_gnn", "geometry_att_gnn", "graph_transformer"]:
            self.modality = "graph"
            with open_dict(backbone_params):
                backbone_params.update({
...
class MixedDiffusionModel(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        if backbone in ["att_gnn", "geometry_att_gnn", "graph_transformer"]:
            with open_dict(backbone_params):
                backbone_params.update({
...
class ContinuousDiffusionModel(nn.Module):
    ...
    def reverse_guidance_force(self, x_hat, cond, t, mask=None):
        ...
        for _ in range(self.grad_descent_steps):
            optimizer.zero_grad()
            legality_weight, hpwl_weight = self.get_scheduled_guidance_weights(t)
            h_legality = legality_weight * guidance.legality_guidance_potential(x_hat_current, cond, mask=mask, softmax_factor=legality_softmax_factor)
            h_hpwl = hpwl_weight * guidance.hpwl_guidance_potential(x_hat_current, cond)
            h = h_legality + h_hpwl
            h.sum().backward()
            optimizer.step()
    ...
    def get_scheduled_guidance_weights(self, t):
        base_legality_weight = 1.0 if self.guidance_mode == "opt" else self.legality_guidance_weight
        if not _cfg_bool(self.guidance_schedule, "enabled", False):
            return base_legality_weight, self.hpwl_guidance_weight

        progress = _progress_from_t(t)
        legality_only_until = _cfg_float(self.guidance_schedule, "legality_only_until", 0.4)
        joint_until = _cfg_float(self.guidance_schedule, "joint_until", 0.8)

        legality_initial = _cfg_float(self.guidance_schedule, "legality_initial_weight", base_legality_weight)
        legality_mid = _cfg_float(self.guidance_schedule, "legality_mid_weight", max(0.0, base_legality_weight / 3.0))
        legality_final = _cfg_float(self.guidance_schedule, "legality_final_weight", max(0.0, base_legality_weight / 10.0))
        hpwl_initial = _cfg_float(self.guidance_schedule, "hpwl_initial_weight", 0.0)
        hpwl_final = _cfg_float(self.guidance_schedule, "hpwl_final_weight", self.hpwl_guidance_weight)

        if progress <= legality_only_until:
            return legality_initial, hpwl_initial

        denom = max(joint_until - legality_only_until, 1e-6)
        ramp = _smoothstep((progress - legality_only_until) / denom)
        if progress <= joint_until:
            legality = legality_mid + ramp * (legality_final - legality_mid)
            hpwl = hpwl_initial + ramp * (hpwl_final - hpwl_initial)
            return legality, hpwl

        return legality_final, hpwl_final
...
class FlowMatchingModel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # guidance parameters
        self.is_guided_sampling = \
            (legality_guidance_weight > 0.0 or hpwl_guidance_weight > 0.0 or bbox_guidance_weight > 0.0 or heat_repulsion_guidance_weight > 0.0 or alpha_lr > 0.0 or alpha_init > 0.0) \
            and (grad_descent_steps > 0) \
            and (grad_descent_rate > 0.0) \
            and (guidance_mode != "none")
        self.guidance_mode = guidance_mode
        self.legality_guidance_weight = legality_guidance_weight
        self.hpwl_guidance_weight = hpwl_guidance_weight
        self.bbox_guidance_weight = bbox_guidance_weight
        self.heat_repulsion_guidance_weight = heat_repulsion_guidance_weight
        self.bbox_softmax_beta = bbox_softmax_beta
        self.heat_repulsion_sigma = heat_repulsion_sigma
        self.heat_repulsion_power_key = heat_repulsion_power_key
        self.heat_repulsion_normalize_power = heat_repulsion_normalize_power
    ...
    def reverse_guidance_force(self, x_current, cond, t, mask=None):
        ...
        for _ in range(self.grad_descent_steps):
            ...
            h_legality = legality_weight * guidance.legality_guidance_potential(
                x_guided,
                cond,
                mask=mask,
            ...
            h_heat = heat_repulsion_weight * guidance.heat_repulsion_guidance_potential(
                x_guided,
                cond,
                mask=mask,
                sigma=self.heat_repulsion_sigma,
                power_key=self.heat_repulsion_power_key,
                normalize_power=self.heat_repulsion_normalize_power,
            )
            h = h_legality + h_hpwl + h_bbox + h_heat
            h.sum().backward()
            optimizer.step()
    ...
    def get_scheduled_guidance_weights(self, t):
        base_legality_weight = 1.0 if self.guidance_mode == "opt" else self.legality_guidance_weight
        if not _cfg_bool(self.guidance_schedule, "enabled", False):
            return base_legality_weight, self.hpwl_guidance_weight, self.bbox_guidance_weight, self.heat_repulsion_guidance_weight
        ...
        hpwl_initial = _cfg_float(self.guidance_schedule, "hpwl_initial_weight", 0.0)
        hpwl_final = _cfg_float(self.guidance_schedule, "hpwl_final_weight", self.hpwl_guidance_weight)
        bbox_initial = _cfg_float(self.guidance_schedule, "bbox_initial_weight", 0.0)
        bbox_final = _cfg_float(self.guidance_schedule, "bbox_final_weight", self.bbox_guidance_weight)
        heat_initial = _cfg_float(self.guidance_schedule, "heat_repulsion_initial_weight", 0.0)
        heat_final = _cfg_float(self.guidance_schedule, "heat_repulsion_final_weight", self.heat_repulsion_guidance_weight)

        if progress <= legality_only_until:
            return legality_initial, hpwl_initial, bbox_initial, heat_initial
        ...
        if progress <= joint_until:
            legality = legality_mid + ramp * (legality_final - legality_mid)
            hpwl = hpwl_initial + ramp * (hpwl_final - hpwl_initial)
            bbox = bbox_initial + ramp * (bbox_final - bbox_initial)
            heat = heat_initial + ramp * (heat_final - heat_initial)
            return legality, hpwl, bbox, heat

        return legality_final, hpwl_final, bbox_final, heat_final
