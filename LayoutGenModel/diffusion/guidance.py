...
def heat_repulsion_guidance_potential(
        x,
        cond,
        mask=None,
        sigma=0.35,
    ...
    """
    Differentiable heat-source spreading proxy.

    High-power nodes close to each other produce a larger potential, so gradient
    descent pushes nearby high-power chiplets apart. This is intentionally a
    fast physical proxy, not a replacement for the HRNet thermal evaluator.
    """
    if x.dim() == 2:
        x = x.unsqueeze(0)
    ...
    elif hasattr(cond, "is_ports"):
        active = ~cond.is_ports.view(-1).bool().to(device=device)
    else:
        active = torch.ones((V,), dtype=torch.bool, device=device)

    if not bool(active.any()) or int(active.sum().item()) <= 1:
        return x.new_zeros((B,))

    pos = x[:, active, :2]
    if power_key in cond:
        power = cond[power_key].to(device=device, dtype=dtype).view(-1)[active].abs()
    else:
        power = torch.ones((pos.shape[1],), dtype=dtype, device=device)

    if normalize_power == "graph_max":
        power = power / power.max().clamp_min(1e-6)
    elif normalize_power in [None, "none", "None", ""]:
        pass
    else:
        raise ValueError(f"unknown heat repulsion normalize_power={normalize_power}")

    dist2 = torch.cdist(pos, pos, p=2).square()
    sigma2 = max(float(sigma) * float(sigma), 1e-8)
    pair_power = power.view(1, -1, 1) * power.view(1, 1, -1)
    pair_mask = torch.triu(torch.ones((pos.shape[1], pos.shape[1]), dtype=torch.bool, device=device), diagonal=1)
    energy = pair_power * torch.exp(-dist2 / sigma2)
    return energy[:, pair_mask].mean(dim=1)

def _bbox_active_mask(cond, mask=None, device=None):
    if mask is not None:
        active = ~mask.view(-1).bool()
    elif hasattr(cond, "is_ports"):
        active = ~cond.is_ports.view(-1).bool()
    else:
        active = torch.ones(cond.x.shape[0], dtype=torch.bool, device=cond.x.device)
    if device is not None:
        active = active.to(device)
    return active

def bbox_extents(x, cond, mask=None):
    """
    Hard chiplet bounding box in normalized canvas coordinates.
    Ports are excluded by default because they are fixed IO anchors, not chiplets.
    """
    if x.dim() == 2:
        x = x.unsqueeze(0)
        squeeze = True
    ...
    active = _bbox_active_mask(cond, mask=mask, device=x.device)
    if not torch.any(active):
        zeros = x.new_zeros((x.shape[0],))
        result = (zeros, zeros, zeros)
        return tuple(v.squeeze(0) for v in result) if squeeze else result

    pos = x[:, active, :2]
    sizes = cond.x[active, :2].to(device=x.device, dtype=x.dtype).unsqueeze(0)
    left = (pos[..., 0] - sizes[..., 0] / 2).min(dim=1).values
    right = (pos[..., 0] + sizes[..., 0] / 2).max(dim=1).values
    bottom = (pos[..., 1] - sizes[..., 1] / 2).min(dim=1).values
    top = (pos[..., 1] + sizes[..., 1] / 2).max(dim=1).values
    width = right - left
    height = top - bottom
    area = width * height
...
def bbox_area_guidance_potential(x, cond, mask=None, softmax_beta=30.0):
    """
    Differentiable chiplet bounding-box area potential.
    Uses smooth max/min so gradients reach near-boundary chiplets.
    """
    if x.dim() == 2:
        x = x.unsqueeze(0)
        squeeze = True
    ...
    if not torch.any(active):
        zeros = x.new_zeros((x.shape[0],))
        return zeros.squeeze(0) if squeeze else zeros

    pos = x[:, active, :2]
    sizes = cond.x[active, :2].to(device=x.device, dtype=x.dtype).unsqueeze(0)
    beta = float(softmax_beta)
    left_edge = pos[..., 0] - sizes[..., 0] / 2
    right_edge = pos[..., 0] + sizes[..., 0] / 2
    bottom_edge = pos[..., 1] - sizes[..., 1] / 2
    top_edge = pos[..., 1] + sizes[..., 1] / 2

    right = torch.logsumexp(beta * right_edge, dim=1) / beta
    left = -torch.logsumexp(-beta * left_edge, dim=1) / beta
    top = torch.logsumexp(beta * top_edge, dim=1) / beta
    bottom = -torch.logsumexp(-beta * bottom_edge, dim=1) / beta
    width = torch.relu(right - left)
    height = torch.relu(top - bottom)
    area = width * height
    return area.squeeze(0) if squeeze else area
...
def _unique_undirected_edge_ids(edge_index):
    """Return the first edge id for each unordered non-self edge."""
    seen = set()
    keep = []
    edge_index_cpu = edge_index.detach().cpu()
    for edge_id, (src, dst) in enumerate(edge_index_cpu.t().tolist()):
        if src == dst:
            continue
...
def weighted_pairwise_hpwl(x, cond, net_aggr="sum", raw_output=False):
    ...
    Uses one directed entry per undirected connection, independent of edge order.
    """
    if x.dim() == 2:
        x = x.unsqueeze(0)
        squeeze = True
    ...
    edge_ids = _unique_undirected_edge_ids(cond.edge_index)
    unique_edges = int(edge_ids.numel())
    if unique_edges == 0:
    ...
    edge_index = cond.edge_index[:, edge_ids].to(device=x.device)
    edge_attr = cond.edge_attr[edge_ids]
    weights = cond.edge_weight[edge_ids].to(device=x.device, dtype=x.dtype).view(1, unique_edges)
    src = edge_index[0]
    dst = edge_index[1]
...
def compute_pin_map(cond):
    ...
    edge_ids = _unique_undirected_edge_ids(cond.edge_index)
    unique_edges = int(edge_ids.numel())
    edge_index_unique = cond.edge_index[:, edge_ids].T # (E, 2)
    edge_attr_unique = cond.edge_attr[edge_ids, :]
    if "edge_pin_id" in cond:
        edge_pin_id_unique = cond.edge_pin_id[edge_ids, :]
    ...
    pin_edge_index = pin_inverse_index.view(2, unique_edges)
