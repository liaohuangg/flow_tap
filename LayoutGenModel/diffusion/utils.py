...
def _clean_log_part(value):
    text = str(value)
    text = re.sub(r"[^A-Za-z0-9_.@+=-]+", "_", text).strip("._")
    return text or "default"

def _log_base_dir(cfg):
    log_root = cfg.get("log_root", None)
    if log_root not in (None, ""):
        return str(log_root)

    log_dir = Path(str(cfg.get("log_dir", "logs")))
    if log_dir.name in ("model", "output"):
        return str(log_dir.parent if str(log_dir.parent) else Path("logs"))
    if log_dir.parent.name in ("logs", "log"):
        return str(log_dir.parent)
    return str(log_dir)

def run_name(cfg):
    return f"{cfg.task}.{cfg.method}.{cfg.seed}"

def structured_run_dir(cfg, kind):
    return os.path.join(
        _log_base_dir(cfg),
        kind,
        _clean_log_part(cfg.task),
        _clean_log_part(cfg.method),
        f"seed_{_clean_log_part(cfg.seed)}",
    )

def model_log_dir(cfg):
    return structured_run_dir(cfg, "model")

def output_log_dir(cfg):
    return structured_run_dir(cfg, "output")

def legacy_log_dir(cfg):
    return os.path.join(str(cfg.get("log_dir", "logs")), run_name(cfg))

def _legacy_structured_candidate(base_dir, kind, relative_path):
    parts = Path(str(relative_path)).parts
    if not parts:
        return None

    run_part = parts[0]
    split = run_part.rsplit(".", 2)
    if len(split) != 3:
        return None
    task, method, seed = split
    ...
    return os.path.join(
        str(base_dir),
        kind,
        _clean_log_part(task),
        _clean_log_part(method),
        f"seed_{_clean_log_part(seed)}",
        *parts[1:],
    )

def _resolve_relative_artifact(cfg, relative_path, preferred_kind):
    raw = str(relative_path)
    path = Path(raw)
    if path.is_absolute():
        return raw

    base_dir = _log_base_dir(cfg)
    kinds = [preferred_kind] if preferred_kind else []
    kinds.extend(kind for kind in ("model", "output") if kind not in kinds)

    candidates = [
        raw,
        os.path.join(base_dir, raw),
        os.path.join(str(cfg.get("log_dir", "logs")), raw),
    ]
    for kind in kinds:
        candidates.append(os.path.join(base_dir, kind, raw))
        legacy_candidate = _legacy_structured_candidate(base_dir, kind, raw)
        if legacy_candidate is not None:
            candidates.append(legacy_candidate)

    legacy_debug = os.path.join(base_dir, "diffusion_debug", raw)
    candidates.append(legacy_debug)
...
def resolve_checkpoint_path(cfg, from_checkpoint):
    if from_checkpoint in (None, "", "none", "None"):
        return None
    return _resolve_relative_artifact(cfg, from_checkpoint, "model")

def resolve_output_path(cfg, output_path):
    if output_path in (None, "", "none", "None"):
        return None
    return _resolve_relative_artifact(cfg, output_path, "output")
...
@torch.no_grad()
def validate_graph_batches(dataloader, model, split, num_graphs=1):
    """
    Average validation metrics over multiple sampled graphs.

    GraphDataLoader batches currently contain one graph repeated across the
    batch dimension, because different graphs can have different node counts.
    This helper samples several graph batches sequentially and averages the
    scalar metrics, giving a much less noisy train/val snapshot.
    """
    num_graphs = max(1, int(num_graphs))
    model.eval()
    metric_values = {}
    metric_counts = {}
    for _ in range(num_graphs):
        x_val, cond = dataloader.get_batch(split)
        t = torch.randint(1, model.max_diffusion_steps + 1, [x_val.shape[0]], device=x_val.device)
        loss, model_metrics = model.loss(x_val, cond, t)
        ...
        for key, value in logs.items():
            ...
            if scalar is None or not np.isfinite(scalar):
                continue
            metric_values[key] = metric_values.get(key, 0.0) + scalar
            metric_counts[key] = metric_counts.get(key, 0) + 1
        cond.to(device="cpu")
    model.train()
    averaged = {
        key: metric_values[key] / metric_counts[key]
        for key in metric_values
        if metric_counts.get(key, 0) > 0
    }
    averaged["num_graphs"] = float(num_graphs)
    return averaged
...
@torch.no_grad()
def generate_report(num_samples, dataloader, model, logger, policy = "iterative", intermediate_every = 200, eval_function = None):
    metrics = common.Metrics()
    report_eval_function = eval_function if eval_function is not None else eval_samples
    for _ in range(num_samples):
        x_eval, cond_eval = dataloader.get_batch("val")
        x_eval = x_eval[:1]
        sample_metrics = display_graph_samples(1, x_eval, cond_eval, model, logger, prefix = "eval", eval_function = report_eval_function, policy = policy, intermediate_every = intermediate_every)
...
def save_outputs(
    ...
    placed_stem = output_case_stem(cond, idx, "placed")
    sample_stem = output_case_stem(cond, idx, "sample")
    ...
    debug_plot_img(image_legalized, os.path.join(save_folder, placed_stem))
    ...
    save_placement_json(sample, cond_postprocessed, save_folder, idx)
    t3 = time.time()
    ...
    bbox_metric_values = bbox_metrics(sample_unprocessed[0], cond_preprocessed, reference_x=x_preprocessed[0])
    t4 = time.time()
...
def eval_samples(samples, x_val, cond_val, use_new_legality_fn = True):
    ...
    for idx, (sample, x) in enumerate(zip(samples, x_val)):
        ...
        current_metrics = {
            ...
            **bbox_metrics(sample, cond_val, reference_x=x),
        }
...
def load_graph_data_with_config(dataset_name, train_data_limit = None, val_data_limit = None, override_placement_path = None, placement_format = "output*.pickle"):
    ...
    dataset_path = os.path.join(repo_dir, f'datasets/graph/{dataset_name}')
    if not os.path.exists(dataset_path):
        benchmark_path = os.path.join(repo_dir, f'benckmark/{dataset_name}')
    ...
    if os.path.exists(dataset_path):
        ...
        graph_files = {int(re.search(r'\d+', p.name).group()):str(p) for p in Path(dataset_path).rglob("graph*.pickle")}
        placement_files = {int(re.search(r'\d+', p.name).group()):str(p) for p in Path(placement_path).rglob(placement_format)}
...
def load_synthetic_graph_data(dataset_name, train_data_limit = None, val_data_limit = None):
    ...
    if ("num_train_samples" in dataset_config and "num_val_samples" in dataset_config):
        ...
        data_files = {int(re.search(r'\d+', p.name).group()):str(p) for p in Path(dataset_path).rglob("*.pickle")}
...
def bbox_metrics(x, cond, reference_x=None, prefix="bbox"):
    width, height, area = guidance.bbox_extents(x, cond)
    metrics = {
        f"{prefix}_width": float(width.detach().cpu().item()),
        f"{prefix}_height": float(height.detach().cpu().item()),
        f"{prefix}_area": float(area.detach().cpu().item()),
    }
    if reference_x is not None:
        ref_width, ref_height, ref_area = guidance.bbox_extents(reference_x, cond)
        ref_area_value = float(ref_area.detach().cpu().item())
        metrics.update({
            f"original_{prefix}_width": float(ref_width.detach().cpu().item()),
            f"original_{prefix}_height": float(ref_height.detach().cpu().item()),
            f"original_{prefix}_area": ref_area_value,
            f"{prefix}_area_ratio": metrics[f"{prefix}_area"] / max(1e-12, ref_area_value),
        })
    return metrics
...
def write_summary_metrics(output_metrics, filename_base, extra_metrics=None):
    summary = {}
    ...
    if extra_metrics:
        for key, value in extra_metrics.items():
            try:
                ...
                continue
    summary["num_samples"] = int(max((len(v) for v in output_metrics.values()), default=0))
...
def add_summary_metrics(summary_metrics, metrics, prefix=None):
    for key, value in metrics.items():
        ...
        try:
            ...
            continue
        summary_metrics.setdefault(metric_key, []).append(value)
...
def output_case_stem(cond, idx, suffix):
    idx_value = idx
    if isinstance(idx_value, torch.Tensor):
        idx_value = idx_value.detach().cpu().view(-1)[0].item()
    try:
        idx_text = f"{int(idx_value):02d}"
    except (TypeError, ValueError):
        idx_text = _safe_filename_part(idx_value)

    if "benchmark_name" in cond:
        name = cond.benchmark_name
        if isinstance(name, (list, tuple)):
            name = name[0] if name else ""
        return f"{idx_text}_{_safe_filename_part(name)}_{suffix}"
    return f"{suffix}{idx_text}"
...
def _load_benchmark_input_json(cond):
    ...
    if os.environ.get("MTAP_ROOT"):
        candidates.append(Path(os.environ["MTAP_ROOT"]) / "benchmark" / "test_input" / f"{name}.json")
    if os.environ.get("ATPLACE_BENCHMARK_ROOT"):
        candidates.append(Path(os.environ["ATPLACE_BENCHMARK_ROOT"]) / f"{name}.json")
    candidates.extend([
        here.parents[3] / "MTAP" / "benchmark" / "test_input" / f"{name}.json",
        here.parents[2] / "MTAP" / "benchmark" / "test_input" / f"{name}.json",
        here.parents[2] / "benchmark" / "ATPlace_json" / f"{name}.json",
        here.parents[3] / "benchmark" / "ATPlace_json" / f"{name}.json",
    ])
...
def _placement_bbox_stats(chiplets):
    if not chiplets:
        return 0.0, 0.0
    x0 = min(float(ch["x-position"]) for ch in chiplets)
    y0 = min(float(ch["y-position"]) for ch in chiplets)
    x1 = max(float(ch["x-position"]) + float(ch["width"]) for ch in chiplets)
    y1 = max(float(ch["y-position"]) + float(ch["height"]) for ch in chiplets)
    width = max(0.0, x1 - x0)
    height = max(0.0, y1 - y0)
    area = width * height
    aspect_ratio = min(width, height) / max(width, height, 1e-12)
    return area, aspect_ratio

def _placement_wirelength(chiplets, connections):
    ...
    for conn in connections:
        ...
        wirelength += weight * (abs(c1x - c2x) + abs(c1y - c2y))
    return wirelength

def _fallback_placement_json_data(placement, cond):
    sizes = cond.x.detach().cpu().numpy() if isinstance(cond.x, torch.Tensor) else np.asarray(cond.x)
    powers = None
    if "node_power" in cond:
        powers = cond.node_power.detach().cpu().numpy() if isinstance(cond.node_power, torch.Tensor) else np.asarray(cond.node_power)
    chiplets = []
    for i, ((x, y), size) in enumerate(zip(placement, sizes)):
        item = {
            "name": f"C{i}",
            "x-position": float(x),
            "y-position": float(y),
        ...
        if powers is not None and i < len(powers):
            item["power"] = float(powers[i])
        chiplets.append(item)

    connections = []
    if "edge_index" in cond:
        edge_index = cond.edge_index.detach().cpu().numpy() if isinstance(cond.edge_index, torch.Tensor) else np.asarray(cond.edge_index)
        seen = set()
        for e in range(edge_index.shape[1]):
            ...
            if u == v or key in seen:
                continue
            seen.add(key)
            connections.append({"node1": f"C{u}", "node2": f"C{v}"})
    return chiplets, connections

def save_placement_json(placement, cond, save_folder, idx):
    placement_dir = Path(save_folder).resolve().parent / "placement"
    placement_dir.mkdir(parents=True, exist_ok=True)
    out_path = placement_dir / f"{output_case_stem(cond, idx, 'placement')}.json"

    source = _load_benchmark_input_json(cond)
    if source is not None and "chiplets" in source:
        ...
        chiplets, connections = _fallback_placement_json_data(placement, cond)

    area, aspect_ratio = _placement_bbox_stats(chiplets)
    data = {
        "chiplets": chiplets,
        "connections": connections,
        "wirelength": _placement_wirelength(chiplets, connections),
        "area": area,
