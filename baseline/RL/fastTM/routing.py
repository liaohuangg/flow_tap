...
def _load_cplex():
	"""Import an existing CPLEX installation, including sibling Conda envs."""
	try:
		import cplex
		return cplex
	except ModuleNotFoundError as first_error:
		# Training uses the CUDA environment while CPLEX may live in a sibling
		# Conda environment (for example tap25d). Reuse it without reinstalling.
		executable = Path(sys.executable).resolve()
		envs_dir = next(
			(parent for parent in executable.parents if parent.name.lower() == 'envs'),
			None,
		)
		if envs_dir is not None:
			candidates = list(envs_dir.glob('*/lib/python*/site-packages'))
			candidates.extend(envs_dir.glob('*/Lib/site-packages'))
			for candidate in sorted(candidates):
				if (candidate / 'cplex').is_dir():
					sys.path.insert(0, str(candidate))
					import cplex
					return cplex
		raise RuntimeError(
			'IBM CPLEX Python API is required for RLPlanner wirelength. '
			'Install it in the training environment or a sibling Conda environment.'
		) from first_error
...
def solve_Cplex(system):
	...
	try:
		...
		raise RuntimeError('CPLEX failed to produce a routing solution') from exc
